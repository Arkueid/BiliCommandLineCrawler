...
def _download(bv, video=False, audio=False, frames=False, cover=False, part: int | None = None, list_flag=False, cookie: str = ""):
    bv = BV(bv[0], cookie)
    ...
    if list_flag or (not part and (bv.getBType() == BType.EPISODE) and len(info['列表']) > 1):
        ...
        if bv.getBType() == BType.EPISODE or bv.getBType() == BType.BANGUMI:
